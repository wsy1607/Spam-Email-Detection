#This script is for final project part 6, 8 and 9.
...
class.svm <- function(data,d,l){
  ...
  for (i in 1:10){
    ...
    ind <- random.ind[(i*n/10-n/10+1):(i*n/10)]
    #Get SVM model
    mod.svm=svm(y~., data=data[-ind,], kernel ="polynomial",degree=d,cost =l,scale=T,probability=T)
    #Predict
...
#Word feature
wordfeature <- read.csv("word feature.csv",header=T)
#Drop the first column, which is useless.
wordfeature <- wordfeature[,-1]
#Assign variable y as factor, which represents ham or spam.
colnames(wordfeature)[1] <- "y"
wordfeature$y <- as.factor(wordfeature$y)

#Power feature
powerfeature <- read.csv("power feature.csv",header=T)
powerfeature <- powerfeature[,-1]
colnames(powerfeature)[1] <- "y"
powerfeature$y <- as.factor(powerfeature$y)

#Combined feature
combinedfeature <- read.csv("word+power feature matrix.csv",header=T)
combinedfeature <- combinedfeature[,-1]
colnames(combinedfeature)[1] <- "y"
combinedfeature$y <- as.factor(combinedfeature$y)
